...
static t_word	*ft_alias_to_tword(t_word *w, char *val, t_shell *sh, int *doit)
{
	...
	*doit += 1;
	...
	ft_strdel(&e_alias.s);
	if (w_alias && sh->loop + 1 <= 199
		&& (ft_check_in_head(sh->head_al, w_alias->word, sh->loop)))
	{
	...
	while (w && ++i && sh->loop < 199)
	{
			...
			&& (val = get_tenvv_val(sh->alias, w->word)))
		{
			if (ft_strlen(val) > 0 && val[ft_strlen(val) - 1] == ' ')
				i = 0;
			if (sh->head_al[k] != NULL && k <= 199)
				ft_strdel(&sh->head_al[k]);
			if (k <= 199 && (sh->head_al[k] = ft_strdup(w->word)))
